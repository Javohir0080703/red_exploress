...
const DestinationCarts = () => {
  const [active, setActive] = useState("all"); // Boshlang'ich qiymat null
  const continents = [...new Set(allData.map((item) => item.continent))];

  return (
    <section className="pt-16 pb-28">
      <div className="containerb">
        <ul className="flex items-center justify-center flex-wrap mb-[122px]">
          ...
          {allData.map((e, index) => {
            if (active === e.continent) {
              return (
                <li
                  key={index}
                  className="shadow-2xl w-full max-w-[350px] bg-white"
                >
                  {/* Image background */}
                  <div className="">
                    <img className="w-full h-[300px] " src={e.img} alt="img" />
                  </div>
